...
個人健康 Dashboard 的公開介面是唯讀應用，不提供人工輸入、匯入、上傳或修改健康紀錄；未解鎖時不載入或顯示任何健康數字。私人健康摘要與餐食資料另存於受保護資料庫及私人物件儲存，不會隨公開網站程式碼發布。

- 只有獲邀請電郵可使用家庭帳戶登入
- 沒有分析追蹤工具
- 沒有廣告
- 沒有健康資料遙測
- 網站本身不直接連接 Apple Health／HealthKit；只有 iPhone HealthBridge 經使用者授權後可上傳日級聚合
- 背景同步頻率不作保證；介面只顯示私人 API 已成功保存的最近資料
...
虛構健康記錄與飲食照片佔位內容只保留為自動測試 fixtures，不會在未登入的生產介面載入。它們不是 Apple Health 或真實飲食資料，也不代表任何真實個人的健康狀況。公開網站沒有新增或匯入健康資料的入口。
...
家庭 membership 不等於資料分享。客廳 iPad 只可取得每位成員明確授權的最少化摘要，例如健康分數、達標狀態、本週方向及一項非醫療建議。共用畫面不得顯示電郵、體重、體脂、腰圍、餐食詳情、相片、備註、owner ID、裝置資料或憑證。

家庭看板資料 contract 會拒絕未知欄位，避免後端新增欄位時意外披露。未完成 sharing grant 或後端授權時，登入後看板必須 fail closed；未登入時保持鎖定，不顯示成員或健康摘要。共享 session 應可由管理介面撤銷，而且任何成員撤銷 scope 後，不得繼續顯示舊摘要。
